...
import "./styles.scss";
import { timeRangesOverlap, useColors, roundTo } from "../../utils";
import {
  createEvent,
  deleteEvent,
  updateEvent,
...
import { Event as EventType, State, TimeRange, User } from "../../types";
import {
  timeSlotDuration,
  TIME_SLOT_HEIGHT,
  SIDEBAR_WIDTH,
...
const SelectingEvent: React.FC<{
  ...
  const eventTop =
    startTime.diff(timeSlots[0], "hours", true) * TIME_SLOT_HEIGHT;
  const eventHeight =
    Math.max(endTime.diff(startTime, "hours", true), 0.5) * TIME_SLOT_HEIGHT;
  return (
...
const Calendar: React.FC<{}> = () => {
  ...
  const events = useSelector<State, EventType[]>(
  ...
  const dayRatio =
    moment.duration(moment().diff(startTime)).asSeconds() / 86400;
  const endTime = moment(startTime).add(1, "day");
  const timeSlots = Array.from({ length: 24 }).map((_, idx) =>
    moment(startTime).add(idx, "hours")
  );

  const [timeSlotsRef, setTimeSlotsRef] = useState<
  ...
  const yToTime = useCallback(
    (y) => {
      ...
      const numSlots =
        roundTo(actualY - timeSlotsRef.offsetTop, TIME_SLOT_HEIGHT / 4) /
        TIME_SLOT_HEIGHT;
      return moment(startTime).add(numSlots * timeSlotDuration, "hours");
    },
  ...
  return (
    ...
    >
            ...
            timeSlotsRef && (
              <Event
                ...
                onUpdateTimeRange={(timeRange: TimeRange) => {
                  if (
                    event.startTime.diff(timeRange.startTime) !== 0 ||
                    event.endTime.diff(timeRange.endTime) !== 0
                  ) {
      ...
      {selection !== undefined &&
        ((s) => <SelectingEvent timeSlots={timeSlots} timeRange={s} />)(
          selection
        )}
    </div>
  );
